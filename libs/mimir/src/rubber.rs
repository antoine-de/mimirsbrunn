...
impl Rubber {
    ...
    fn put(&self, path: &str, body: &str) -> Result<hyper::client::response::Response, EsError> {
        ...
        let url = self.es_client.full_url(path);
        let result = r#try!(self.http_client.put(&url).body(body).send());
        rs_es::do_req(result)
    }
    fn post(&self, path: &str, body: &str) -> Result<hyper::client::response::Response, EsError> {
        ...
        let url = self.es_client.full_url(path);
        let result = r#try!(self.http_client.post(&url).body(body).send());
        rs_es::do_req(result)
    }
    ...
    pub fn create_index(&self, name: &String, index_settings: &IndexSettings) -> Result<(), Error> {
        ...
        let mut settings_json_value = r#try!(serde_json::from_str::<serde_json::Value>(&settings)
    ...
    pub fn publish_index<T: MimirObject>(
        ...
        let last_indexes = r#try!(self.get_last_index(&index, dataset));
        ...
        for<'de> T: MimirObject + serde::de::Deserialize<'de> + std::fmt::Debug,
    {
        ...
        loop {
            let page = r#try!(scan.scroll(&mut self.es_client, &Duration::minutes(1)));
        ...
        r#try!(scan.close(&mut self.es_client));
        Ok(result)
    }
}
