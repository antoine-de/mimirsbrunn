...
impl Rubber {
    ...
    pub fn create_index(&mut self) -> Result<(), curl::ErrCode> {
        self.client.delete_index(&self.index_name);  // TODO handle error
        ...
        where T: Encodable, I: Iterator<Item = T>
    {
        ...
        loop {
            ...
            chunk.push(Action::index(json::encode(&addr).unwrap()).with_index(self.index_name.clone()).with_doc_type(type_name));

            nb += 1;
            for addr in iter.by_ref().take(1000) {
                //chunk.push(Action::index(json::encode(&addr).unwrap()).with_index(self.index_name).with_doc_type(type_name));
                nb += 1;
            }
            self.client.bulk(&chunk).send().unwrap(); //TODO use result
        }
